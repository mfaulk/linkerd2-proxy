...
fn retry_uses_budget() {
    profile_test! {
        ...
        with_metrics: |metrics: client::Client| {
            ...
            assert!(
                !metrics.get("/metrics").contains("skipped=\"timeout\""),
                "skipped=timeout metrics should not exist"
            );
        }
    }
}
...
fn does_not_retry_if_past_timeout() {
    profile_test! {
        ...
        with_metrics: |metrics: client::Client| {
            ...
            assert!(
                !metrics.get("/metrics").contains("skipped=\"budget\""),
                "skipped=budget metrics should not exist"
            );
        }
    }
}
